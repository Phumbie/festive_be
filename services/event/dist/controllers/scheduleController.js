"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.addSchedule = addSchedule;
exports.listSchedules = listSchedules;
exports.updateSchedule = updateSchedule;
exports.deleteSchedule = deleteSchedule;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
function toScheduleResponse(schedule) {
    return {
        ...schedule,
        date: schedule.date instanceof Date ? schedule.date.toISOString() : schedule.date,
        createdAt: schedule.createdAt instanceof Date ? schedule.createdAt.toISOString() : schedule.createdAt,
    };
}
async function addSchedule(req, res) {
    try {
        const schedule = await prisma.schedule.create({
            data: {
                eventId: req.params.eventId,
                ...req.body,
            },
        });
        res.status(201).json(toScheduleResponse(schedule));
    }
    catch (err) {
        res.status(500).json({ error: 'Failed to add schedule', details: err });
    }
}
async function listSchedules(req, res) {
    try {
        const schedules = await prisma.schedule.findMany({
            where: { eventId: req.params.eventId },
        });
        res.json(schedules.map(toScheduleResponse));
    }
    catch (err) {
        res.status(500).json({ error: 'Failed to list schedules', details: err });
    }
}
async function updateSchedule(req, res) {
    try {
        const schedule = await prisma.schedule.update({
            where: { id: req.params.scheduleId },
            data: req.body,
        });
        res.json(toScheduleResponse(schedule));
    }
    catch (err) {
        res.status(500).json({ error: 'Failed to update schedule', details: err });
    }
}
async function deleteSchedule(req, res) {
    try {
        await prisma.schedule.delete({ where: { id: req.params.scheduleId } });
        res.status(204).send();
    }
    catch (err) {
        res.status(500).json({ error: 'Failed to delete schedule', details: err });
    }
}
