...
exports.assignAdminRoleToUser = assignAdminRoleToUser;
...
async function createRole(req, res) {
    ...
    if (permissions && Array.isArray(permissions)) {
        const invalidPermissions = permissions.filter(perm => !SYSTEM_PERMISSIONS.find(sysPerm => sysPerm.name === perm));
 ...
 */
async function initializeDefaultRoles(req, res) {
    try {
        ...
        res.json({
            ...
            roles: {
                admin: {
                    ...
                    permissionsCount: result.adminRole?.permissions?.length || 0
                },
                projectManager: {
                    id: result.projectManagerRole?.id,
                    name: result.projectManagerRole?.name,
                    description: result.projectManagerRole?.description,
                    permissionsCount: result.projectManagerRole?.permissions?.length || 0
                }
            },
